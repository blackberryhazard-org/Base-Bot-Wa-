...
async function handleCommand(conn, m, command, args, fullText, isButton) {
  ...
  if (!target) {
    return reply('Contoh: .addowner 628xxx')
  }

  const ownerDB = readJSON(ownerPath)
  const num = getNumber(target)

  if (ownerDB.includes(num)) {
    return reply('Sudah jadi owner')
  }
  ...
  if (!target) {
    return reply('Contoh: .delowner 628xxx')
  }
  ...
  if (!target) {
    return reply('Contoh: .addprem 628xxx')
  }

  const premiumDB = readJSON(premiumPath)
  const num = getNumber(target)

  if (premiumDB.includes(num)) {
    return reply('Sudah premium')
  }
  ...
  if (!target) {
    return reply('Contoh: .delprem 628xxx')
  }
